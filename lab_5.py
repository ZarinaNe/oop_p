class User:
    def __init__(self, name, login, password, bd):
        self.__name = name  # приватные поля доступны только внутри класса и его методов
        self.__bd = bd # название базы данных в которой хранится пользователь
        self.__login = login
        self.__password = password
        self.__autorisation = False  # Состояние авторизованности сохранил ли пользователь куки для быстрого входа
    ...
    def _getBd(self):  #объект является внутренним и предназначен только для использования внутри класса, модуля или функции
        return self.__bd

    def getAutorization(self):
        return self.__autorisation
